(() => {
    ...
    function handleExpenseSubmit(e) {
        ...
        if (editingExpenseId) {
            ...
            expenseForm.querySelector('button[type="submit"]').innerHTML = '<i class="fas fa-plus"></i> Add Expense';
        } else {
        ...
        showToast(editingExpenseId ? 'Expense updated successfully!' : 'Expense added successfully!');
    }
    ...
    function updateSummaryCards() {
        ...
        const monthlyTotal = getMonthlyExpenses(currentMonth).reduce((sum, exp) => sum + exp.amount, 0);
    ...
    function updateBudgetProgress() {
        ...
        if (percentage > 100) {
            ...
            progressText.textContent = `${Math.round(percentage)}% used (Over budget!)`;
    ...
    function displayExpenses() {
        ...
        if (selectedMonth) {
            filteredExpenses = filteredExpenses.filter(exp => exp.date.startsWith(selectedMonth));
        }
        ...
        expensesList.innerHTML = filteredExpenses.map(expense => `
            <div class="expense-item fade-in">
                ...
                <div class="expense-amount">₹${expense.amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
    ...
    window.editExpense = function(expenseId) {
        ...
        expenseForm.querySelector('button[type="submit"]').innerHTML = '<i class="fas fa-save"></i> Update Expense';
    ...
    window.deleteExpense = function(expenseId) {
        ...
        const expenseElement = document.querySelector(`[onclick*="${expenseId}"]`).closest('.expense-item');
    ...
    function drawLegend(ctx, categories, categoryData, total) {
        ...
        categories.forEach((category, index) => {
            ...
            // Draw text
            ctx.fillStyle = '#333';
            ctx.fillText(`${getCategoryName(category)} (${percentage}%)`, legendX + 25, legendY);
    ...
    function getCategoryData() {
        ...
        const selectedMonth = monthFilter.value;
        if (selectedMonth) {
            filteredExpenses = expenses.filter(exp => exp.date.startsWith(selectedMonth));
        }
